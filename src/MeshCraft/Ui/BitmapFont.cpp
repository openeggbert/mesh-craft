...
// 5×7 column-major bitmap font for ASCII 0x20–0x7E.
// Each entry is 5 bytes (one per column, left→right).
...
static constexpr int kGlyphGap = 1; // pixels between characters
    ...
    for (unsigned char ch : text) {
        ...
        for (int col = 0; col < kGlyphW; ++col) {
            ...
            for (int row = 0; row < kGlyphH; ++row) {
                if (colBits & (1u << row)) {
                    fillRect(curX + col * scale,
                             y    + row * scale,
                             scale, scale, color);
                }
            }
        }
        curX += (kGlyphW + kGlyphGap) * scale;
    }
}
...
int bitmapTextWidth(const std::string& text, int scale)
{
    ...
    return static_cast<int>(text.size()) * (kGlyphW + kGlyphGap) * scale - kGlyphGap * scale;
}
