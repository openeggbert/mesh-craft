...
static std::string serializeScene(const Mc3::Mc3Document& doc) {
    ...
    auto tmp = fs::temp_directory_path() / "mc_ai_scene.mc3.xml";
    doc.saveToFile(tmp);
    std::ifstream f(tmp);
...
// Parse an mc3.xml string into a document.
static Mc3::Mc3Document parseXml(const std::string& xml) {
    namespace fs = std::filesystem;
    auto tmp = fs::temp_directory_path() / "mc_ai_response.mc3.xml";
...
void MeshCraftApplication::drawAiPanel() {
    ...
    if (ImGui::Button("Send", ImVec2(90, 0))) {
        ...
        try {
            std::string sceneXml = serializeScene(document_);
    ...
    if (hasDone && !aiAssistant_.hasError()) {
        ...
        if (ImGui::Button("Apply to Scene", ImVec2(130, 0))) {
            try {
                ...
                Mc3::Mc3Document newDoc = parseXml(xml);
                pushUndo();
        ...
        // Optional: Save first definition to registry
        if (registry_.isOpen() && !document_.definitions.empty()) {
            ImGui::SameLine();
            if (ImGui::Button("Save to Registry…")) {
                showRegistryPanel_ = true;
                regSaveDlgOpen_    = true;
                // Pre-fill name from prompt (first word)
                std::string word;
                for (char c : std::string(aiPromptBuf_))
                    if (c == ' ' || c == '\n') break; else word += c;
                std::strncpy(regSaveNameBuf_, word.c_str(), sizeof(regSaveNameBuf_) - 1);
    ...
    if (hasDone) {
        ImGui::SameLine();
        if (ImGui::Button("Reset")) aiAssistant_.reset();
    }
