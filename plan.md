...
| STAB-0653 | 📋 | P1 | Remove the dead root-level `default_camera` attribute from `mc3.xsd` — never implemented, `<cameras default="...">` is the real mechanism | `mc3/mc3.xsd:905` | `mc3.xsd:905` declares `<mc3 default_camera="...">`, but it's never read in `Mc3XmlParser.cpp`, never written in `Mc3XmlWriter.cpp`, and `Mc3Document.hpp` has no field for it — completely dead. The working mechanism is `<cameras default="...">` → `doc.defaultCamera` (`Mc3XmlParser.cpp:448,465-466`, `Mc3XmlWriter.cpp:439-440`). Per current policy ("no new features until stabilization gates are green"), implementing a second parallel mechanism is out of scope — the correct fix is removing the unimplemented promise from the schema. Verify: `xsd_validation` still 20/20 after removal (no fixture uses the attribute); grep confirms no other reference. |
| STAB-0654 | 📋 | P1 | Implement the `mip_maps` texture attribute — documented and schema-declared, but silently dropped on load (real data loss) | `mc3/mc3.xsd:641`, `mc3/include/MeshCraft/Mc3/Mc3Texture.hpp`, `mc3/src/Mc3XmlParser.cpp` (`parseTextures`, ~L469-494), `mc3/src/Mc3XmlWriter.cpp` | `mc3.xsd:641` declares `mip_maps` (`xs:boolean default="true"`) on `<texture>`. `Mc3Texture` has no field for it, `parseTextures()` never reads it, the writer never emits it — an authored `mip_maps="false"` is silently and permanently lost on load. Fix: add `bool mipMaps{true}` to `Mc3Texture`, parse it in `parseTextures()`, write it only when `!= true` (matching the file's existing omit-default convention). Verify: round-trip test — load a fixture with `mip_maps="false"`, save, reload, confirm it survived. |
| STAB-0655 | 📋 | P2 | Fix `version` attribute default mismatch — XSD says `"0.3"`, parser/model fall back to `"0.1"` | `mc3/mc3.xsd:898`, `mc3/src/Mc3XmlParser.cpp:895`, `mc3/include/MeshCraft/Mc3/Mc3Document.hpp:30`, `MC3_FORMAT.md:11,18` | `mc3.xsd:898` declares `default="0.3"`. `Mc3XmlParser.cpp:895` falls back to `"0.1"` when the attribute is absent from a loaded file; `Mc3Document.hpp:30`'s in-class default is also `"0.1"`, matching root `MC3_FORMAT.md`'s own opening example/table (lines 11/18) — which is itself inconsistent with the same doc's later example (line 65, `version="0.3"`). Confirmed nothing branches on `doc.version` (it's purely round-tripped/stored — `ModelRegistry.cpp:261`, `McbWriter.cpp:449`, `GltfExporter.cpp:1127`), so this is low-impact but a genuine 3-way inconsistency. Fix: change the parser fallback and `Mc3Document.hpp`'s default to `"0.3"` (matching the XSD, the file actually used for validation); fix `MC3_FORMAT.md`'s opening example/table to also read `0.3`. Verify: a `.mc3.xml` omitting `version` now round-trips as `doc.version == "0.3"`; 66/66 ctest unaffected. |
| STAB-0656 | 📋 | P3 | Gate `<uv_mapping>` write/parse by object type to match XSD scoping (group/instance/area objects can't have it per-schema) | `mc3/src/Mc3XmlWriter.cpp:105`, `mc3/src/Mc3XmlParser.cpp` (`parseCommonObjectAttribs`, ~L249-260), `mc3/mc3.xsd` (`instanceType:471-481`, `areaType:483-492`, `objectsContainerType:496+` — none declare `uv_mapping`, unlike all 11 primitive types + mesh + extrude, which do) | The writer emits `<uv_mapping>` for ANY object type whenever `obj->uvMapping` is set; the parser likewise reads it for any type via the shared `parseCommonObjectAttribs()`. If a container/instance/area object ever ends up with a non-null `uvMapping` (likely unreachable via the current editor UI, but not structurally prevented), the writer would emit schema-invalid XML. Fix: gate the write in `Mc3XmlWriter.cpp` by `obj->type` (skip for Group/Union/Difference/Intersection/Instance/Area). Low urgency — narrow edge case. Verify: 66/66 ctest unaffected; confirm no existing fixture regresses. |
| STAB-0657 | 📋 | P3 | Delete stale duplicate `mc3/MC3_FORMAT.md` (misleading — documents non-working `mip_maps`, contradicts current root `MC3_FORMAT.md`); add missing `star` cross-section mention to the canonical doc | `mc3/MC3_FORMAT.md` (delete), `MC3_FORMAT.md:508` (extend) | `mc3/MC3_FORMAT.md` (last touched 2026-06-21) is an orphaned, unreferenced duplicate of the root `MC3_FORMAT.md` (last touched 2026-07-09) — nothing in the build, README, or code points to it (only an incidental narrative mention in `STABILIZATION_WORKLOG.md`'s log of a past check). It actively misleads: it documents `mip_maps` as if functional (see STAB-0654) and claims "all sections are optional and may appear in any order," which the XSD's strict root sequence contradicts. Separately, the canonical root `MC3_FORMAT.md:508` lists only rect/circle/polygon/custom cross-section types, omitting the fully-implemented `star` type. Fix: `git rm mc3/MC3_FORMAT.md`; add `star` to the root doc's cross-section list (fold in `sides`/`cx`/`cy`/`cz` documentation alongside STAB-0651/0652 if convenient). Verify: no remaining functional reference to the deleted file; root doc's extrude section lists all 5 cross-section types. |
...
| S21 Format audit (new 2026-07-10) | 7 | 2 | 0 | 0 | 5 | 0 |
| **TOTAL** | **657** | **622** | **29** | **0** | **6** | **0** |
