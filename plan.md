...
_New section, added 2026-07-10 after 6 parallel deep-audit passes over `mc3togltf` (geometry/primitives, CSG, materials/textures, animation, scene structure/cameras/lights/instancing, and glTF spec compliance/robustness) — does it produce correct, spec-compliant, high-fidelity glTF, and does the maximum of the mc3 format actually make it into the export? 38 genuine, individually-verified findings, not "hundreds" — see the task-count-honesty note at the end of this section for why, and what a path to a much larger count would actually look like. Every P0 and several P1 findings below were independently re-verified directly against the source by reading the exact cited lines before being added here (not just trusted from the audit fork), continuing this session's established discipline after an earlier fork this session wrongly claimed a root XML attribute was completely dead when 13 real fixtures used it (STAB-0653)._

| ID | St | Pri | Title | Key File(s) | Verification |
|----|----|-----|-------|-------------|--------------|
| STAB-0662 | 📋 | P1 | `buildCapsule()`'s bottom-hemisphere math overshoots past the pole — geometry doesn't close cleanly, and diverges from the editor's own capsule builder | `mc3togltf/src/MeshBuilder.cpp:363`, `src/MeshCraft/Renderer/SceneRenderer_Builders.cpp:419-493` | `phi = (pi*0.5f) + (pi*0.5f)*float(r-rings)/rings` reaches `rings+1` (not `rings`) on the last row given `totalRows = 2*rings+2`, so `phi` overshoots `pi` (e.g. `segments=16` → last-row `phi=202.5°`, `sinP≈-0.383`, a sign-flipped radial component instead of closing to a point). `SceneRenderer_Builders.cpp`'s independent capsule builder uses a correct construction (explicit pole vertices, hemisphere loops terminating exactly at the pole) — every exported capsule's glTF geometry differs from the editor preview. Fix: correct the `phi` formula so the last row is exactly `pi`. Verify: a capsule export's bottom-pole vertices should all coincide at one point (zero radial spread); add a geometry test asserting this. |
| STAB-0663 | 📋 | P1 | IcoSphere has an unhandled UV seam (texture stretches across ~1 meridian) | `mc3togltf/src/MeshBuilder.cpp:489-548` | Vertices are shared via midpoint-caching across the whole subdivided mesh, with exactly one equirectangular UV (`atan2`/`asin`) per unique position — any triangle straddling the `atan2` discontinuity gets UVs on opposite ends of `[0,1]`. `buildSphere()` in the same file already solves this correctly (duplicates the seam column, `s <= sectors` inclusive) — apply the same fix to IcoSphere. Verify: export an IcoSphere with a checker texture, confirm no full-width UV-stretch triangles (e.g. assert no triangle has a texcoord U-span > 0.5). |
...
| STAB-0665 | 📋 | P2 | `test/extrude_sides_bezier.mc3.xml` (added this session, STAB-0651/0652) is never exercised by any mc3togltf test — non-default polygon/star `sides` and bezier-path extrusion geometry has never been verified to actually export valid, non-degenerate glTF | `mc3togltf/test/`, `test/extrude_sides_bezier.mc3.xml`, `mc3togltf/src/MeshBuilder.cpp:578-757` (`sampleCrossSection()`/`samplePath()`) | This fixture was added to fix an XSD schema gap, but the actual export-time geometry math for non-default-sided polygon/star cross-sections and Catmull-Rom bezier paths is genuinely complex and has zero export-side test coverage. Fix: add an mc3togltf test exporting this fixture, asserting non-zero triangle count, finite (non-NaN) vertex positions, and valid accessor bounds for all 3 objects (Hexagon/FivePointStar/BezierRibbon). Verify: the new test passes and would have caught a plausible regression in the sampling math. |
| STAB-0666 | 📋 | P2 | Primitive dual-path cross-check (`all_primitives_export_test.py::test_bbox_matches_scene_renderer_scale_formula`) only compares bounding boxes, not vertex count/topology — proven too weak to catch a real bug (STAB-0662) | `mc3togltf/test/all_primitives_export_test.py` | The existing test directly covers capsule bbox and did NOT catch STAB-0662's pole-closure bug, since a folded/pinched pole doesn't change the overall bounding box. Fix: extend the cross-check to also compare vertex count (or a topology fingerprint) between `mc3togltf`'s and `SceneRenderer`'s geometry for each primitive. Verify: the strengthened test would fail against the pre-STAB-0662 code and pass after the fix (test this by temporarily reverting STAB-0662 locally, or via a synthetic bad-geometry case). |
| STAB-0667 | 📋 | P2 | `loadObjMesh()` ignores per-face OBJ `material_ids` — multi-material OBJ imports silently flatten to one material | `mc3togltf/src/MeshBuilder.cpp:1042-1127` | Any OBJ with multiple materials/groups per-face is flattened under mc3's single `<mesh material="...">` id, silently discarding per-face OBJ material assignments (`shape.mesh.material_ids` is never read). `obj_robustness_test.py` covers only crash-safety, not multi-material content. Fix: either split a multi-material OBJ import into multiple glTF primitives (one per material) within the same mesh, or document this as a deliberate accepted limitation if splitting is out of scope. Verify: import a 2-material test OBJ, confirm both materials are represented distinctly in the export (not just the first/last one silently winning). |
| STAB-0668 | 📋 | P2 | `buildCone()` produces NaN normals when `radius=0` and `height=0` simultaneously | `mc3togltf/src/MeshBuilder.cpp:234-235` | `slopeLen = sqrt(radius²+height²)` is 0 when both are 0; `ny = radius/slopeLen` is then `0/0` = NaN. A degenerate-dimensioned cone (plausible from bad import data or an editor typo) silently produces NaN normals — `GltfExporter.cpp`'s finite-value guard only covers the OBJ-import path's POSITION accessor, not primitive NORMAL accessors. Fix: guard against `slopeLen < epsilon` and fall back to a sane default normal direction. Verify: export a zero-radius zero-height cone, confirm no NaN in the output glTF's NORMAL accessor. |
| STAB-0669 | 📋 | P2 | `buildSphere()` pole rings produce degenerate (zero-area) triangles instead of a proper fan | `mc3togltf/src/MeshBuilder.cpp` (sphere pole rows) | Both poles collapse all sector vertices to one 3D position; the triangle-strip indexing still emits `segments` zero-area triangles per pole, unlike the cylinder/cone caps which correctly use fan-based pole geometry. Usually harmless (zero-area triangles are typically culled/ignored by renderers) but worth fixing for cleanliness and to avoid tripping strict-validator zero-area warnings. Fix: use fan-based pole triangulation matching cylinder/cone's existing correct pattern. Verify: exported sphere has no zero-area triangles (cross-product-based area check in a test). |
| STAB-0670 | 📋 | P0 | Torus/Capsule/IcoSphere silently vanish from the CSG **editor live preview** (produce an empty manifold) but export correctly — a real WYSIWYG break | `src/MeshCraft/Renderer/SceneRenderer.cpp:83-143` (`buildManifoldTree()`, no `case` for Torus/Capsule/IcoSphere, falls to `default: return Manifold{}`), `mc3togltf/src/CsgEvaluator.cpp:222-247` (correctly has explicit cases) | **Verified directly**: `SceneRenderer.cpp`'s `switch(obj.type)` at line 83 has cases for Box/Cube/Sphere/Cylinder/Cone/Union/Intersection/Difference/Group/Area/Instance only — Torus/Capsule/IcoSphere (and Plane/Disk/Grid/Mesh/Extrude) fall through to the empty-manifold default. `CsgEvaluator.cpp` correctly handles Torus/Capsule/IcoSphere via `buildPrimitive()`. `MC3_FORMAT.md:554` documents these as supported CSG children. Impact: a `<difference>` using a Torus cutter previews as "nothing subtracted" in the editor, then exports correctly — undetected because no fixture exercises these types as CSG children on either path. Fix: add the missing `case` branches to `buildManifoldTree()`, building the same primitives `MeshBuilder`/`CsgEvaluator` do. Verify: new `test/csg_*.mc3.xml` fixture with a Torus cutter; assert the editor's CSG preview cache produces a non-empty, correctly-shaped result matching the export. |
| STAB-0671 | 📋 | P2 | Curved-primitive tessellation mismatch between CSG preview (24 segments) and export (32 segments) — previews visibly less smooth than final output | `src/MeshCraft/Renderer/SceneRenderer.cpp:72` (`SEG=24`), `mc3togltf/src/CsgEvaluator.cpp:171` (`CSG_SEGMENTS=32`) | Not wrong, but a quality/consistency gap — Sphere/Cylinder/Cone CSG previews are visibly less smooth than the final export. Fix: share one constant between both files (or make preview match export's 32). Verify: visual — no automated test needed beyond confirming the constants match after the fix. |
...
| STAB-0673 | 📋 | P2 | CSG determinism is completely untested | `mc3togltf/test/determinism_test.py`, `mc3togltf/CMakeLists.txt:459-465` | `determinism_test.py` runs only against `test/house.mc3.xml`, which has zero CSG operations (confirmed via grep). Manifold boolean ops are floating-point-sensitive; repeat-export byte-identity for CSG content has never been verified. Fix: add a CSG-bearing fixture to the determinism test's coverage (two independent process invocations, byte-for-byte comparison, matching the existing test's own sound methodology). Verify: the new determinism check passes for a CSG-bearing scene. |
| STAB-0674 | 📋 | P2 | CSG root material assignment is untested | `mc3togltf/test/csg_export_test.py`, `MC3_FORMAT.md:565` | `MC3_FORMAT.md:565` documents "the CSG root's material is used for the entire merged mesh," but `csg_export_test.py` never asserts the merged mesh's actual material index. Fix: add an assertion. Verify: test fails if the documented behavior regresses. |
...
| STAB-0676 | 📋 | P0 | GLB texture embedding hardcodes MIME type to `image/png` for every embedded image, regardless of actual format | `mc3togltf/src/GltfExporter.cpp:305` | **Verified directly**: `img.mimeType = "image/png";` is unconditional, with a comment confirming the developer's assumption ("already-encoded PNG bytes") rather than actual detection. Any JPEG (or other non-PNG) texture embedded into a GLB export gets tagged as PNG — spec-compliant loaders will fail to decode the image (JPEG bytes parsed as PNG). No JPEG fixture exists anywhere in `test/` (confirmed), and `glb_texture_embed_test.py:74` only asserts a `data:image/png;base64,...` prefix, actively encoding the PNG-only assumption rather than catching the gap. Fix: detect image format from `tex.uri`'s extension or magic bytes, set `mimeType` to `"image/png"` or `"image/jpeg"` accordingly. Verify: add a JPEG texture fixture, confirm the embedded GLB's `mimeType` is `"image/jpeg"` and the image data round-trips (decodable) through a real loader (tinygltf or Blender). |
| STAB-0677 | 📋 | P1 | Plain `.gltf` (non-GLB) export never rebases external texture URIs against the *output* file's directory | `mc3togltf/src/GltfExporter.cpp:313` | `img.uri = tex.uri;` uses the raw mc3-XML-source-relative URI unchanged. Everywhere else in the codebase, `tex.uri` is resolved relative to `doc.sourcePath` — but the non-embed write path never re-relativizes it against the output directory. If a `.gltf` is exported to a directory other than the source `.mc3.xml`'s own directory (a common workflow, e.g. exporting into `dist/`), the written texture URI is wrong/unresolvable. No existing test exercises cross-directory export with an external texture. Fix: compute the texture's absolute path from `doc.sourcePath`, then re-relativize against the output `.gltf`'s directory before writing `img.uri`. Verify: export to a different directory than the source, confirm the resulting `.gltf`'s texture URI actually resolves to the real file. |
| STAB-0678 | 📋 | P2 | `Mc3Texture::colorSpace` is parsed/stored but never read anywhere in `mc3togltf` — sRGB-vs-linear correctness for normal/metallicRoughness/occlusion textures is entirely unenforced and undocumented | `mc3togltf/src/*.cpp` (confirmed zero matches via full-file grep), `MC3_FORMAT.md:219` | glTF spec requires `baseColorTexture`/`emissiveTexture` sRGB and `normalTexture`/`metallicRoughnessTexture`/`occlusionTexture` linear — no pixel-level re-encoding happens on export, so correctness is entirely the source image file's responsibility, with zero validation. This may be an acceptable pass-through design, but it's undocumented (`MC3_FORMAT.md:219` shows `color_space="srgb"` in an example with no explanation of what it does/doesn't affect at export time). Fix: at minimum, document the pass-through behavior clearly in `MC3_FORMAT.md`; consider a build-time or export-time warning if a texture used as a normal/MR/occlusion map has `colorSpace="srgb"` explicitly set (likely an author mistake). Verify: doc update reviewed; if a warning is added, test it fires for the mismatched case. |
| STAB-0679 | 📋 | P3 | Dangling `material="foo"` reference is silently dropped with no diagnostic, inconsistent with the analogous dangling-SVG-texture case (which does warn) | `mc3togltf/src/GltfExporter.cpp:563-567` | `matIdx` stays `-1`, node ends up with no material — low severity (glTF permits materialless primitives) but the silent-vs-warned inconsistency should be normalized. Fix: add a stderr warning matching the existing `warnIfUnresolvedSvg` pattern. Verify: export a scene with a dangling material reference, confirm a warning is printed. |
| STAB-0680 | 📋 | P1 | Visibility animation (`AnimatedProperty::Visible`) has no glTF export path — silently dropped, no fallback attempted, undocumented | `mc3togltf/src/GltfExporter.cpp:933-949` (generic unsupported-property `default:` case) | Falls into the same bucket as material-color/deform animation (which genuinely have no glTF equivalent), but "object appears/disappears at time T" is a common, plausible authoring pattern unlike those. Fix: at minimum, consider a scale-to-zero fallback (animate `scale` to `(0,0,0)` when invisible) as a widely-compatible visual approximation, or explicitly document this as an accepted limitation if a fallback is out of scope. Verify: whichever direction is chosen, add a test proving the chosen behavior (fallback animation present, or a clear warning + documented limitation). |
| STAB-0681 | 📋 | P2 | `Mc3Action::autoplay`/`loop` are completely and silently lost on export — inconsistent with this file's own established `extras`-preservation convention | `mc3togltf/src/GltfExporter.cpp` (animation export section) | Confirmed via grep: never written anywhere. Inconsistent with this exact file's own convention of preserving otherwise-inexpressible mc3 data via glTF `extras` (node tags/collision/mc3_type, light castShadows, scene environment, asset mc3_version all already do this). Fix: add `autoplay`/`loop` to the exported animation's `extras` object, matching the established pattern. Verify: round-trip test — export, reload the glTF, confirm `extras.autoplay`/`extras.loop` are present and correct. |
| STAB-0682 | 📋 | P2 | Dangling animation `targetObject` references fail silently with zero diagnostic, inconsistent with the unsupported-property case 15 lines away (which does warn) | `mc3togltf/src/GltfExporter.cpp:961-962` | `if (nodeIt == nodeNameMap.end()) continue;` — a typo'd target-object name produces a channel that silently does nothing, with no warning naming the channel (unlike the unsupported-property skip path nearby, which does print a clear warning). Fix: add a stderr warning naming the dangling `targetObject`. Verify: export a scene with a typo'd channel target, confirm a warning is printed. |
| STAB-0683 | 📋 | P2 | Cubic-bezier animation baking uses a hardcoded, uncapped 30 samples/sec rate — no adaptive resampling, no cap on resulting accessor size | `mc3togltf/src/GltfExporter.cpp:990` | `t += 1.0f / 30.0f`, fixed rate with no configurability or documented rationale. Long bezier-interpolated actions could produce very large accessors purely from the fixed rate (no cap); very fast/sharp curves in a short window could under-sample. Only a coarse `count > 2` smoke check exists — the rate choice and its extremes are untested. Fix: consider adaptive resampling (denser sampling where curvature/velocity is high) or at least a documented, sane cap on total samples per channel. Verify: a stress test with a very long bezier action confirms output size stays bounded; a very short/sharp one confirms visual fidelity is still reasonable (sampled comparison against `evaluateChannel()`'s exact values at several intermediate times). |
| STAB-0684 | 📋 | P3 | `animation_unsupported_test.py` exercises only 2 of the 12 `AnimatedProperty` values sharing the generic skip-with-warning code path | `mc3togltf/test/animation_unsupported_test.py`, `mc3/include/MeshCraft/Mc3/Mc3Animation.hpp` (`AnimatedProperty` enum) | Only `material.baseColor.r` and `deform.x` are tested; the other 10 (Visible — see STAB-0680, DeformY/Z, MaterialBaseColorG/B/A, MaterialRoughness/Metallic, MaterialEmissiveR/G/B) have no individual regression proving the property-name-to-warning-string mapping is correct for each. Fix: parametrize the existing test over all 12 values. Verify: extended test passes for all 12. |
| STAB-0685 | 📋 | P3 | `evaluateChannel()` requires pre-sorted keyframes (`std::upper_bound`) with no validation anywhere — affects `mc3togltf`'s bezier-baking derivatively since it calls this function, but the fix belongs in the `mc3` library | `mc3/src/Mc3Animation.cpp:87` | Documented only as a header comment, not enforced. If violated (an author or programmatic scene-builder inserts out-of-order keyframes), interpolation silently produces wrong results — both in the live editor AND in mc3togltf's export (since export calls the same function for bezier baking). Fix: either sort keyframes defensively inside `evaluateChannel()`, or validate-and-warn at parse/save time in the `mc3` library (this is a `mc3`-library-scoped fix, file it there, not in `mc3togltf`). Verify: a test with intentionally-out-of-order keyframes confirms either correct sorting or a clear warning, not silent wrong interpolation. |
| STAB-0686 | 📋 | P1 | Duplicate object names silently misdirect animation targeting and base-transform lookups | `mc3togltf/src/GltfExporter.cpp:908` (`collectBaseTransforms`), `:1172-1176` (`nodeNameMap`) | Both maps are keyed by `obj->name`/`node.name` with last-write-wins on collision, no uniqueness check, no warning. `exportAnimations` resolves `Mc3Channel::targetObject` (a name string, not an ID) through this map — if two sibling objects share a name, only the later-processed one receives the animation/correct base transform; the other is silently unanimated. Nothing in the mc3 data model enforces name uniqueness (that's `id`'s job) — this is a real, reachable authoring scenario. Fix: at minimum, warn when a duplicate name is detected during node-map construction; consider preferring `id`-based targeting if `Mc3Channel` can be extended to support it (larger, product-scope change — file as `needs_human` if so). Verify: export a scene with two same-named objects where only one is animated, confirm a warning fires and document which one "wins" (or fix to disambiguate correctly). |
| STAB-0687 | 📋 | P1 | `buildMesh()`'s TEXCOORD_0/NORMAL accessor creation is unconditional, unlike its sibling `addMeshDataToGltf()` which correctly guards TEXCOORD_0 — risk of a spec-invalid zero-count accessor if `MeshBuilder` ever produces UV-less/normal-less geometry | `mc3togltf/src/GltfExporter.cpp:466-468` (unconditional), `:506` (sibling's correct guard) | **Verified directly**: lines 466-468 call `addAccessorVec3(model, md.normals)`/`addAccessorVec2(model, md.texcoords)` with no `!empty()` guard, while `addMeshDataToGltf()` at line 506 has `if (!md.texcoords.empty())`. glTF's accessor schema requires `count >= 1`; if geometry generation (see STAB-0662/0663/0667/0668/0669's geometry-audit siblings) ever produces empty normals/texcoords for this path, this emits spec-invalid output instead of gracefully omitting the attribute. Fix: apply the same `!empty()` guards used in `addMeshDataToGltf()`. Verify: confirm (with the geometry audit's findings) whether this path is actually reachable with empty normals/texcoords today; add the guard regardless as defense-in-depth, with a test if a reachable case is found. |
| STAB-0688 | 📋 | P1 | Export determinism is verified only against a feature-poor fixture — CSG, animation, and texture-embedding reproducibility are completely unverified | `mc3togltf/CMakeLists.txt:459-465`, `mc3togltf/test/determinism_test.py`, `test/house.mc3.xml` | The test methodology is sound (two independent process invocations, byte-for-byte comparison) but `house.mc3.xml` has zero CSG operations, zero animations, and zero textures (confirmed via grep). Floating-point-sensitive Manifold CSG evaluation, keyframe/interpolation evaluation, and texture/image-byte embedding are all completely unverified for reproducibility. Overlaps with STAB-0673 (CSG-specific) but is broader — also covers animation and texture embedding. Fix: extend `determinism_test.py`'s fixture list (or add cases) to include CSG-, animation-, and texture-bearing content. Verify: new determinism checks pass for each added content type. |
...
| STAB-0691 | 📋 | P0 | `doc.rotationUnits`/`doc.eulerOrder` are completely ignored in glTF export — every export assumes degrees + extrinsic XYZ order regardless of the document's actual declared convention | `mc3togltf/src/MathUtils.hpp:10` (`eulerXYZToQuat()`, hardcoded degrees + XYZ composition), `mc3togltf/src/GltfExporter.cpp:555,844,1015` (all 3 call sites) | **Verified directly**: `eulerXYZToQuat()`'s parameters are named `pitchDeg`/`yawDeg`/`rollDeg` and its own comment says "Extrinsic XYZ" — both hardcoded assumptions. Confirmed zero grep matches for `rotationUnits`/`eulerOrder` anywhere in `GltfExporter.cpp`/`MathUtils.hpp`. Any document declaring `rotation_units="radians"` or a non-XYZ `euler_order` exports systematically wrong rotations for every rotated object/camera, with no error. (The same two fields were also found entirely missing from the MCB binary format this session, `STAB-0658` — a different code path, this bug is independent and unfixed.) Fix: read `doc.rotationUnits`/`doc.eulerOrder` once at export start, convert to radians if needed, and generalize `eulerXYZToQuat()` (or add sibling functions) to compose in the document's actual declared axis order, not a hardcoded one. Verify: export a scene with `rotation_units="radians"` and a non-default `euler_order`, confirm the resulting glTF quaternion matches an independently-computed reference rotation. |
| STAB-0692 | 📋 | P0 | Spot light cone angle is exported at exactly **half** its authored value — divides by 360 instead of 180 | `mc3togltf/src/GltfExporter.cpp:756` | **Verified directly**: `Mc3Light.hpp:24` documents `angle` as "half-angle in degrees" (correct conversion: `angle * pi / 180`). `GltfExporter.cpp:756` does `light.angle * pi / 360.0` — dividing by 360 halves the resulting radian value. Cross-verified against the editor's own independent spot-light math, `SceneRenderer.cpp:1055`: `li.angle * (pi/180.0f)` — confirms the exporter is wrong by exactly 2×. Every exported spotlight's glTF cone is half as wide as the editor preview and the source file's authored value. Fix: change `/ 360.0` to `/ 180.0` at line 756. Verify: export a spot light with a known angle, confirm the glTF `spot.outerConeAngle` (in radians) matches `angle_degrees * pi/180`, not half that. |
| STAB-0693 | 📋 | P1 | Camera node positions are never unit-scaled — `addCameraNodes()` has no `unitScale` parameter at all, unlike every other node type | `mc3togltf/src/GltfExporter.cpp:808-810` (function signature, no scale param), `:1164` (call site, only 3 args), contrast `:1178` (`exportAnimations` correctly passes `ctx.unitScale`) | **Verified directly**: `addCameraNodes(tinygltf::Model&, const std::vector<Mc3Camera>&, std::vector<int>&)` — 3 parameters, no `unitScale`. Called at line 1164 as `addCameraNodes(model, doc.cameras, cameraNodes);`. Every other node's translation IS scaled by `ctx.unitScale` (lines 708-711, converting `unit="centimeter"`/`"inch"` documents to metres). A camera in a non-meter-unit document is positioned using un-converted raw values while the geometry around it is correctly scaled — the camera ends up at the wrong distance from the scene. Fix: add a `unitScale` parameter to `addCameraNodes()` and apply it to `cam.position`, matching the pattern used elsewhere. Verify: export a `unit="centimeter"` scene with a camera at a known position, confirm the glTF camera node's translation is scaled by 0.01 like everything else. |
| STAB-0694 | 📋 | P1 | Perspective camera `aspectRatio` is hardcoded to 16:9 for every camera, regardless of the actual intended output | `mc3togltf/src/GltfExporter.cpp:821` | **Verified directly**: `gcam.perspective.aspectRatio = 16.0 / 9.0;` unconditional, no per-camera field read. Per glTF spec, omitting `aspectRatio` lets a viewer adapt to its actual rendering viewport — hardcoding it forces every camera to a fixed ratio, which can visibly distort framing in spec-respecting viewers that honor the declared ratio over their own window shape. Fix: either omit `aspectRatio` entirely (letting viewers adapt, the spec-recommended default), or add an actual aspect-ratio field to `Mc3Camera` if per-camera control is wanted (a small data-model addition — check whether this counts as "new feature" under current policy, may need a scope decision). Verify: whichever direction, confirm the exported glTF either omits `aspectRatio` or uses a real per-camera value, not a hardcoded constant. |
...
| STAB-0696 | 📋 | P1 | Ambient lights are silently dropped from every glTF export with zero warning | `mc3togltf/src/GltfExporter.cpp:733` | **Verified directly**: `if (light.type == LightType::Ambient) continue; // no ambient in glTF 2.0` — the code comment explains the (correct) reasoning (`KHR_lights_punctual` genuinely has no ambient light type) but there's no `std::cerr` warning or `ctx.stats.warnings++`, unlike the CSG approximate-mode path which does warn for a comparable "can't fully represent this" case. An author's ambient light vanishes with no indication anything was omitted. Fix: add a warning matching the CSG-approximate-mode pattern; optionally consider approximating via a very-low-intensity directional light or `KHR_materials_unlit`-adjacent technique as a documented, opt-in fallback (larger scope, may need a decision). Verify: export a scene with an ambient light, confirm a warning is printed naming the dropped light. |
| STAB-0697 | 📋 | P1 | `Mc3Light::brightness` is passed straight through as glTF `intensity` with no per-light-type unit conversion (lux for directional vs candela for point/spot per `KHR_lights_punctual`) | `mc3togltf/src/GltfExporter.cpp:742` | `lo["intensity"] = tinygltf::Value(brightness);` — no scaling logic for either unit. Physically-based glTF viewers will render lights at wrong absolute brightness relative to each other (though many non-PBR viewers treat intensity as an arbitrary multiplier and may look visually fine regardless — lower real-world impact than STAB-0691/0692, hence P1 not P0). Fix: document mc3's `brightness` as an arbitrary/unitless multiplier if that's the intended semantics (matching current behavior, cheapest fix), or add real per-type unit conversion if physically-correct output across renderers is wanted (larger scope). Verify: whichever direction, `MC3_FORMAT.md`'s light documentation should state clearly what `brightness` means relative to glTF's lux/candela expectation. |
| STAB-0698 | 📋 | P1 | `Mc3Environment::skyboxTexture` is missing from the glTF export's `scene.extras` — exact parallel to `STAB-0659` (MCB), but this is the separate glTF-export code path and is still unfixed | `mc3togltf/src/GltfExporter.cpp:864-896` (`applyEnvironment()`) | `applyEnvironment()` writes `backgroundColor`/`backgroundTexture`/`fog` to `extras` but never `env->skyboxTexture`. Same root cause pattern as `STAB-0659` (an equirectangular-panorama skybox field silently dropped), but a completely independent code path — fixing MCB did not fix this. Fix: add `skyboxTexture` to the `extras` object, matching `backgroundTexture`'s handling. Verify: round-trip test — export a scene with `skyboxTexture` set, reload the glTF, confirm `extras.skyboxTexture` is present and correct. |
...
| S23 mc3togltf export audit (new 2026-07-10) | 38 | 0 | 0 | 0 | 38 | 0 |
| **TOTAL** | **699** | **631** | **29** | **0** | **39** | **0** |
