...
### AUD-009 `[TODO]` `P2` `W2` · Provider JSON parsed by hand-rolled string scanning: extractFirstTextValue trusts the first literal "text":" and mis-decodes UTF-16 surrogate pairs into invalid UTF-8
...
### AUD-010 `[TODO]` `P2` `W2` · No test covers a malicious <include> in an AI response — the arbitrary-file-read path is entirely unverified
...
### AUD-017 `[TODO]` `P3` `W6` · Enum fields are cast from unvalidated file ints with no range check
...
### AUD-018 `[TODO]` `P3` `W6` · MCB_FORMAT.md top-level field-order list is stale (omits rotationUnits, eulerOrder, includedEmbeds)
...
### AUD-019 `[TODO]` `P3` `W6` · Compression-flag rejection path has no test despite being a documented guarantee
...
### AUD-026 `[TODO]` `P2` `W7` · --stats 'Warnings' count is untruthful — several warning paths never increment stats.warnings
...
### AUD-027 `[TODO]` `P1 (self-assigned P2, elevated/adjusted per adversarial correction)` `W7` · Pivoted object + translation animation loses the pivot offset — object jumps when its translation is animated
...
### AUD-028 `[TODO]` `P1 (self-assigned P2, elevated/adjusted per adversarial correction)` `W7` · Rotation animation exported only as LINEAR/STEP quaternion samples (never CUBICSPLINE); euler-space keyframe interpolation is replaced by quaternion shortest-path lerp
...
### AUD-029 `[TODO]` `P3` `W7` · Per-object metadata is silently dropped from glTF, unlike tags/collision which are exported to node extras
...
### AUD-032 `[TODO]` `P2` `W4` · loadPrefsAlg mirror omits the input clamping that production loadPrefs performs — a hand-edited prefs.ini with out-of-range values is silently accepted
...
### AUD-033 `[TODO]` `P2` `W4` · MeshCraftPrivate.hpp keeps byte-identical duplicates of five EditorAlgorithms.hpp helpers, and both copies are compiled into the same translation units
...
- **Resolved:** commits `0dfcd4f`, `d16c82c`, and this session's doc-reconciliation commit (`git log --oneline -- plan.md NEXT.md test/validate_plan_consistency.py` for the exact hash) — verify: `python3 test/validate_plan_consistency.py`
...
- **Resolved:** this session's doc-reconciliation commit (`git log --oneline -- RELEASE.md docs/history/plan_deep_audit.md` for the exact hash) — verify: `grep -c "66/66" RELEASE.md; test -f docs/history/plan_deep_audit.md && echo archived` (RELEASE.md now points at `ctest -N`; plan_deep_audit.md archived as historical, all its own tasks completed)
...
- **Evidence:** AUD-036 (commit 737af77) fixed the specific dead `if(IsItemActivated()) pushUndo()`-nested-in-changed-block pattern at all 80 identified sites, with a source-lint regression guard (undo_snapshot_lint_test.py) that only detects that one syntactic pattern in 3 files. It is not a behavioral test — it has never driven a real ImGui frame or confirmed pushUndo captures pre-mutation state, has never verified Slider/Checkbox/Combo/InputText/drag-and-drop/multi-object-mutation buttons preserve correct undo/redo document values, and has not triaged the 78 candidates test/undo_coverage_audit.py reports (real mutation / transient preview / false positive / intentionally non-undoable — unclassified). No central transaction abstraction exists; every call site is an independent hand-written `_undoChNNN` local (or, for non-drag widgets, a bare `pushUndo()` call with no verification it fires before the mutation it protects).
- **Outcome:** (1) Triage all undo_coverage_audit.py candidates into the four categories, with a written classification per candidate. (2) For each mutating widget category (Drag*/Slider*/Input*/ColorEdit/ColorPicker/Checkbox/Combo/RadioButton/drag-and-drop/multi-object-mutation buttons), verify with a frame-driven or extracted-controller test that pushUndo captures the PRE-mutation document, not a state that has already changed. (3) Design and adopt a central transaction helper (e.g. a scoped `UndoTransaction` RAII type or a `beginEdit()/commitEdit()` pair) so future call sites cannot reintroduce the dead-pattern class of bug, replacing the current ad-hoc per-site local-variable pattern where practical. (4) Verify: one user gesture = one snapshot (no per-frame drag spam), no snapshot for a no-op gesture where practical, redo invalidated on the first real mutation, cancel/revert leaves no false history item, multi-selection edits are atomic, selection restored correctly after undo/redo, locked objects untouched.
