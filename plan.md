...
| STAB-0675 | ✅ | P3 | `csg_stress_test.py` only stress-tests Union-of-boxes — no scale test for Difference/Intersection with many cutters, nested mixed ops, or curved primitives | `test/csg_stress.mc3.xml`, `mc3togltf/test/csg_stress_test.py` | **Fixed 2026-07-10.** Added 4 new fixtures to `csg_stress.mc3.xml`: `ManyCuttersDifference` (a single Difference with 8 sphere cutters, vs. TenBoxUnion's wide *union* count), `IntersectionStress` (an Intersection of 6 overlapping spheres, exercising repeated Manifold intersection beyond the existing 2-3-child fixtures elsewhere), `NestedMixedOps` (a single tree combining all three operators — Difference of a Union base and an Intersection cutter — vs. DeepUnion's same-operator-only nesting), and `CurvedUnionStress` (a union of 12 curved non-box primitives — sphere/cylinder/cone/icosphere — deliberately excluding Torus/Capsule per `STAB-0700`'s tracked separate bug). Each new test assertion checks a real geometric property (bbox extent), not just "didn't crash" — e.g. `IntersectionStress`'s bbox is asserted smaller than a single sphere's diameter (confirms a genuine non-empty intersection, not a silent empty/fallback result), `ManyCuttersDifference`/`NestedMixedOps`/`CurvedUnionStress`'s bboxes are asserted to match their expected full footprint (confirms nothing silently dropped). Whole export (now 6 CSG trees plus the original 2) still completes in ~0.3s. Full 84/84 ctest green. |
...
| STAB-0689 | ✅ | P2 | Zero test coverage for partial-failure behavior of the known-non-atomic multi-file `.gltf` export | `mc3togltf/test/multi_file_partial_failure_test.py` (new), `mc3togltf/src/GltfExporter.cpp:1367-1376` | **Fixed 2026-07-10 — characterized and locked in current behavior, no code change (the imperfect behavior is an acceptable, self-healing tradeoff, not worth fixing now).** Read tinygltf's own `WriteGltfSceneToFile()` source: it writes buffers/images to disk **first**, then the JSON file **last** — this predicts (and empirical testing confirmed) two distinct, asymmetric outcomes: (a) if the `.bin` write is blocked (tested via a pre-existing directory at that path), the function returns `false` before ever attempting the JSON write — **no `.gltf` file is left on disk at all**, a clean/safe failure; (b) if the `.gltf` JSON write is blocked (bin write already succeeded), **a complete, valid `.bin` file IS left on disk with no corresponding `.gltf`** — this is the real, asymmetric non-atomic gap the code comment warns about. Judged this an acceptable tradeoff: it's an orphaned-but-harmless file (not a `.gltf` that looks valid but references a missing/broken buffer, which would be worse), no viewer would load or reference it without the JSON, and a successful retry to the same path overwrites it. New `mc3togltf_multi_file_partial_failure` ctest asserts both directions explicitly, so a future regression that makes the JSON-write-failure case worse (e.g. a corrupt partial `.bin` instead of a complete orphaned one) would be caught. Full 83/83 ctest green. |
| STAB-0690 | ✅ | P3 | GLB binary chunk assembly (header/JSON-padding/BIN-padding byte-level correctness) has no direct test — delegated entirely to tinygltf | `mc3togltf/test/glb_padding_test.py` (new) | **Fixed 2026-07-10.** New test asserts header magic/version/declared-length-matches-file-size, and both chunks' 4-byte alignment and correct padding byte values (JSON: `0x20`, BIN: `0x00`), reading the real exported GLB directly. **Investigated whether BIN padding is even reachable first**: it isn't, currently — every value written into the shared binary buffer is float32 (positions/normals/texcoords) or uint32 (`addAccessorIndices()` always uses `UNSIGNED_INT`, never a smaller component type), both inherently 4-byte aligned, so the buffer's total length is always already a multiple of 4; embedded GLB textures go into the JSON as base64 data URIs, not appended to the BIN buffer, so they don't affect its alignment either. The test still asserts the BIN-padding invariant (real, just not currently forceable non-zero by any actual export) and logs which case it hit rather than silently claiming full coverage. JSON padding, by contrast, IS genuinely exercised non-trivially (2 real `0x20` padding bytes confirmed on the default fixture). Full 84/84 ctest green. |
...
| STAB-0699 | ✅ | P2 | Large-scene glTF export scalability (time/memory growth with object count) was flagged as unaudited, not as "fine" — needs an actual verification pass | `MC3_FORMAT.md` (new "Export scalability" section) | **Verified 2026-07-10 — confirmed good, no code change needed.** Profiled real export time and peak RSS (`/usr/bin/time -v`) for scenes of 1,000 through 50,000 top-level primitive objects (box/sphere/cylinder/cone round-robin, deliberately NOT using `<instance>`/definitions, since instanced geometry is deduplicated via `buildDefCacheKey()` and wouldn't stress the actual worst case of independent `buildMesh()` calls). Both time/object and RSS/object stay flat (0.018-0.020ms, 2.1-8.4KB) all the way to 50,000 objects — linear scaling confirmed directly, no quadratic-or-worse growth found. 50,000-object export completes in under 1 second. Documented the measured table in `MC3_FORMAT.md`. No test added (this is a one-time profiling verification, not a behavior contract — the existing `large_scene_generated_test.py::STAB-0244` 30s-budget assertion at 500 objects already guards against a future regression). |
| STAB-0700 | ✅ | P1 | **New, discovered while implementing STAB-0670**: `buildTorus()`'s raw triangulation is genuinely non-manifold in a way `MeshGL::Merge()` (simple seam-vertex welding) does not fix — Torus has never actually worked as a real-CSG child on either the editor-preview or export path | `mc3togltf/src/CsgEvaluator.cpp:239-264`, `src/MeshCraft/Renderer/SceneRenderer.cpp:122-136`, `test/csg_torus_cutter.mc3.xml`, `test/csg_torus_cutter_test.py` | **Fixed 2026-07-10 — not a topological defect, a tolerance-too-tight bug.** Root-caused with a standalone instrumented probe (compiled directly against the built `libmc3togltf_lib.a`/`libmanifold.so`, dumping `MeshGL::mergeFromVert`/`mergeToVert` counts): confirmed exact-position matching finds **zero** true duplicate vertices among Torus's 561 raw positions (ordinary floating-point trig roundoff means the seam "duplicates" are merely near-identical, never bit-identical, as expected) — the real count of near-duplicate pairs needing a weld is exactly 49 (561 raw − 512 unique grid positions), but `gl.Merge()`'s own auto-computed default tolerance only found 40 of them, leaving 9 seam pairs unwelded and the mesh non-manifold. Sweeping explicit `gl.tolerance` values from `1e-6` to `1e-2` all found the full 49 and produced a valid Manifold (`status=0`) — confirmed this doesn't over-merge by testing a 100x-smaller-scale torus (major_radius=0.004) at the same tolerance: still exactly 49 merges, correct proportionally-tiny volume, and confirmed it doesn't regress the already-working Capsule/IcoSphere cases at the same tolerance. Fixed by setting `gl.tolerance = 1e-4f;` before `gl.Merge()` in both `CsgEvaluator.cpp` (export) and `SceneRenderer.cpp` (editor preview, kept identical to the export path per this session's established dual-path-parity discipline). **Confirmed this is fully independent of `STAB-0702`'s winding fix** (also landed this session) — tested before STAB-0702 that the tolerance fix alone resolves `NotManifold`; winding direction and topological manifold-ness are unrelated properties. New `test/csg_torus_cutter.mc3.xml` + `csg_torus_cutter_test.py` (mirrors the existing Capsule/IcoSphere cutter fixtures): confirms the editor's CSG preview triangle count (1036) exceeds a plain box's (12). Full 85/85 ctest green. |
...
| STAB-0702 | ✅ | P1 | **New, discovered while implementing STAB-0666**: `buildCone()`'s bottom cap and `buildTorus()`'s ring quads wind opposite to their adjacent side faces within the same closed mesh — a real backface-culling correctness bug (default glTF material is single-sided) | `mc3togltf/src/MeshBuilder.cpp` (`buildSphere()`, `buildCone()`, `buildTorus()`, `buildCapsule()`), `mc3togltf/test/all_primitives_export_test.py` (new `test_face_winding_matches_normals()`) | **Fixed 2026-07-10 — scope grew from 2 primitives to 4 during investigation.** Root-caused Cone directly (not just from the STAB-0666 audit's aggregate volume-sign data): built a standalone numeric probe computing each triangle's winding-derived face normal and comparing it against the analytically-correct outward normal at that point — confirmed the side fan (`{apex, apex+1, apex+2}`) is wound backwards while the bottom cap is already correct; fixed by reordering to `{apex, apex+2, apex+1}`. **For Torus, discovered the original STAB-0666 aggregate-volume diagnosis was measuring the wrong thing**: a torus is NOT star-shaped from the origin (it has a hole), so the divergence-theorem-from-origin per-triangle sign is mathematically guaranteed to split into a mix REGARDLESS of whether the mesh's actual winding is correct or backwards (verified directly: flipping Torus's winding entirely still produced a mixed pos/neg split, just different proportions) — the "384/640 mixed" signal in STAB-0666 was a topology artifact, not a reliable defect indicator. The same local per-triangle-vs-own-normal probe used for Cone found Torus's winding IS uniformly backwards (not mixed) for every one of its 1024 triangles; fixed by swapping its shared quad pattern from `{i0,i2,i1, i1,i2,i3}` to `{i0,i1,i2, i1,i3,i2}`. **Applying the same local probe to the other 11 primitives (not just the two originally named) surfaced two more real, previously-undiscovered instances of the identical bug**: `buildSphere()` was uniformly backwards on ALL 960 triangles, and `buildCapsule()` on 1056 of 1088 (the rest were pre-existing zero-area pole triangles, a `STAB-0669`-class defect `buildCapsule()` never got — extended that same pole-fan fix to it too, dropping its count to 1024 and eliminating the last of the winding-check's false readings from degenerate geometry). Fixed both with the identical index-swap. **Verified via a new, general, topology-agnostic test** (`test_face_winding_matches_normals()`, compares each triangle's winding-derived normal against its own stored vertex normals — unlike STAB-0666's origin-based check, this has no star-shaped limitation and correctly validates Torus too): all 13 primitives now show zero mismatches. Separately confirmed this fix does **not** resolve `STAB-0700` (Torus's non-manifold CSG bug) — re-tested a Box-minus-Torus CSG difference after this fix and it still fails `NotManifold`; winding direction and topological manifold-ness are independent properties, as expected. Also added `Cone` to `STAB-0666`'s `CLOSED_SOLID_VOLUMES` now that it's both fixed and (unlike Torus) genuinely star-shaped, so it passes that check too. Full 84/84 ctest green. |
...
| S23 mc3togltf export audit (new 2026-07-10) | 41 | 37 | 0 | 0 | 4 | 0 |
| **TOTAL** | **702** | **668** | **29** | **0** | **5** | **0** |
