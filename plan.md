...
_New section, added 2026-07-10 after 6 parallel deep-audit passes over `mc3togltf` (geometry/primitives, CSG, materials/textures, animation, scene structure/cameras/lights/instancing, and glTF spec compliance/robustness) — does it produce correct, spec-compliant, high-fidelity glTF, and does the maximum of the mc3 format actually make it into the export? 40 genuine, individually-verified findings (38 from the audit + `STAB-0700`/`STAB-0701`, both discovered while implementing fixes for `STAB-0670`/`STAB-0691` respectively), not "hundreds" — see the task-count-honesty note at the end of this section for why, and what a path to a much larger count would actually look like. Every P0 and several P1 findings below were independently re-verified directly against the source by reading the exact cited lines before being added here (not just trusted from the audit fork), continuing this session's established discipline after an earlier fork this session wrongly claimed a root XML attribute was completely dead when 13 real fixtures used it (STAB-0653)._
...
| S23 mc3togltf export audit (new 2026-07-10) | 40 | 5 | 1 | 0 | 34 | 0 |
| **TOTAL** | **701** | **636** | **30** | **0** | **35** | **0** |
