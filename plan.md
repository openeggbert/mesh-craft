...
_New section, added 2026-07-10 after 6 parallel deep-audit passes over `mc3togltf` (geometry/primitives, CSG, materials/textures, animation, scene structure/cameras/lights/instancing, and glTF spec compliance/robustness) — does it produce correct, spec-compliant, high-fidelity glTF, and does the maximum of the mc3 format actually make it into the export? 39 genuine, individually-verified findings (38 from the audit + `STAB-0700`, discovered while fixing `STAB-0670`), not "hundreds" — see the task-count-honesty note at the end of this section for why, and what a path to a much larger count would actually look like. Every P0 and several P1 findings below were independently re-verified directly against the source by reading the exact cited lines before being added here (not just trusted from the audit fork), continuing this session's established discipline after an earlier fork this session wrongly claimed a root XML attribute was completely dead when 13 real fixtures used it (STAB-0653)._
...
| STAB-0670 | 🟡 | P0 | Torus/Capsule/IcoSphere silently vanish from the CSG **editor live preview** (produce an empty manifold) but export correctly — a real WYSIWYG break | `src/MeshCraft/Renderer/SceneRenderer.cpp:106-127`, `mc3togltf/src/CsgEvaluator.cpp:239-248`, `test/csg_icosphere_cutter.mc3.xml`, `test/csg_icosphere_cutter_test.py` | **Fixed 2026-07-10, but revealed two deeper pre-existing bugs while implementing — only IcoSphere is fully fixed end-to-end.** Added the missing `case` branches to `buildManifoldTree()`, reusing `mc3togltf_lib`'s `buildPrimitive()` (same triangulation as the real export, not a re-derived approximation). While testing with a real fixture (none had ever existed for this combination on either path), discovered that constructing a `Manifold` from `buildPrimitive()`'s raw output fails `NotManifold` for Torus and Capsule — **this affects `CsgEvaluator.cpp` (export) too, not just the new preview code**, meaning these two types never actually worked in CSG export either, undetected for the same reason (zero test coverage). Added `MeshGL::Merge()` (welds UV-seam-duplicated vertices) to both `SceneRenderer.cpp` and `CsgEvaluator.cpp` — this fully fixes **IcoSphere** (verified end-to-end: export exits 0, editor preview triangle count rises from 12 (plain box) to 92 (real cut)). **Capsule** still fails — expected, its root cause is the already-tracked, not-yet-implemented `STAB-0662` (pole doesn't close, leaving a non-watertight boundary); should resolve once that's fixed, re-test then. **Torus** still fails for a distinct, newly-discovered reason `Merge()` doesn't fix (30 vertices genuinely merged, still `NotManifold` afterward — not simple seam duplication) — tracked as new `STAB-0700`, not yet root-caused. Regression fixture uses IcoSphere only (the one that fully works); Torus/Capsule deliberately excluded pending their own fixes. Full 69/69 ctest green. |
...
| S23 mc3togltf export audit (new 2026-07-10) | 39 | 3 | 1 | 0 | 35 | 0 |
| **TOTAL** | **700** | **634** | **30** | **0** | **36** | **0** |
