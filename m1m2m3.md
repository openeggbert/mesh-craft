...
- Search bar → live query (matches name, group, tags, description)
...
User types prompt  →  AiAssistant::sendAsync(prompt, currentXml)
                  ...
                  auto-validate: extract XML, parse, reject if empty
...
public:
    ...
    std::string model = "claude-sonnet-4-6";  // UI default; user-editable
...
- On response: auto-validates XML (checks `<mc3>` root, rejects empty docs)
...
Response arrives (each frame, poll() called)
  → if isDone() && !hasError() && !aiPendingDoc_ && !aiValidationError_:
      extract XML, check <mc3> root, parse, reject if empty
