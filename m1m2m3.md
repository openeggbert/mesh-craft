...
Save to Registry (button active when aiPendingDoc_ has definitions)
...
at least one definition. The user can save to the registry **without** applying the
AI result to the current scene. The save dialog is pre-filled with
`source = "ai_generated"` and the first definition id as the name.
