# Archived plan.md rows (2026-07-18)

**This is a historical snapshot, not the active backlog.** These are the `AUD-###`/`SYS-###` rows that were `DONE` as of 2026-07-18 and were moved out of `plan.md` verbatim (full evidence/resolution text, unedited) to keep the active backlog file down to just its still-open rows. See [`plan.md`](../../plan.md) for anything still open, and [`docs/history/README.md`](README.md) for this project's other archived plans.

---

## Systematic workstream tasks (SYS-###) — archived rows
...
## Audit-derived tasks (AUD-###) — archived rows
