...
Copy-Properties, Convert-to-Definition, and Export-Subtree items plus the
Group/Ungroup pair. The detailed Snap interval contents and the remaining
...
- **Last full build: clean after the current Phase 13 Edit-export-subtree
...
- **In progress (2026-07-25):** `SYS-W3-01` Phase 13 moved the concrete
  ...
  selection-action groups, the Select-by-Type/Tag/Material submenus, and the
  Copy-Properties, Convert-to-Definition, and Export-Subtree items plus the
  Group/Ungroup pair are now component-owned. Bookmark, walk, document, undo,
  clipboard, selection, grid, preferences, command-palette, and shortcut-dialog
...
partitions cleanly: 147/147 non-render tests (including `mc3_ai` in 1.34
...
Edit-select-by-type/tag/material, Edit-copy-properties, Edit-grouping, and
Edit-convert-to-definition and Edit-export-subtree slices.
...
Break Instance item, passing only instance-selection availability and one
action callback. Selection/type inspection, definition lookup and object
replacement, undo, document mutation, status reporting, and the
command-palette route would remain application-owned. Alignment actions and
the View-menu Bloom/SSAO block remain outside that slice. Per `CLAUDE.md`,
describe and confirm the Break-Instance slice before implementing it.
